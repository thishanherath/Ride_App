...
function MapScreen() {
  ...
  const initializeMap = () => {
    if (mapRef.current && window.google) {
      ...
      // Load nearby captains
      loadNearbyCaptains(center.lat, center.lng);
    }
  };

  const loadNearbyCaptains = async (lat, lng) => {
    try {
      const token = localStorage.getItem("token");
      const serverUrl = import.meta.env.VITE_SERVER_URL || 'http://localhost:5000';
      
      // This would be a new endpoint to get nearby captains
      // For now, we'll simulate with some mock data
      const mockCaptains = [
        { id: 1, lat: lat + 0.001, lng: lng + 0.001, name: "Captain 1", vehicle: "car" },
        { id: 2, lat: lat - 0.002, lng: lng + 0.001, name: "Captain 2", vehicle: "bike" },
        { id: 3, lat: lat + 0.001, lng: lng - 0.002, name: "Captain 3", vehicle: "auto" },
      ];
      
      setNearbyCaptains(mockCaptains);
      addCaptainMarkers(mockCaptains);
    } catch (error) {
      Console.error("Error loading nearby captains:", error);
    }
  };

  const addCaptainMarkers = (captains) => {
    if (!map) return;

    captains.forEach(captain => {
      const marker = new window.google.maps.Marker({
        position: { lat: captain.lat, lng: captain.lng },
        map: map,
        title: `${captain.name} - ${captain.vehicle}`,
        icon: {
          url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
            <svg width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
              <circle cx="20" cy="20" r="18" fill="#10B981" stroke="white" stroke-width="4"/>
              <path d="M20 8 L24 16 L32 16 L26 22 L28 30 L20 24 L12 30 L14 22 L8 16 L16 16 Z" fill="white"/>
            </svg>
          `),
    ...
    try {
      ...
      geocoder.geocode({ location: { lat, lng } }, (results, status) => {
        if (status === 'OK' && results[0]) {
          ...
          addCaptainMarkers(nearbyCaptains);
  ...
  const handleSuggestionClick = async (suggestion) => {
    try {
      ...
      // Load nearby captains for new location
      loadNearbyCaptains(lat, lng);
  ...
  const handleBookRide = () => {
    if (selectedLocation) {
      ...
      navigate(`/home?pickup=${encodeURIComponent(selectedLocation.address)}`);
    }
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <div className="flex items-center space-x-1 text-sm text-gray-600">
              ...
              <span>{nearbyCaptains.length} drivers nearby</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-4 right-4 bg-white bg-opacity-90 backdrop-blur-sm rounded-lg p-3 shadow-lg">
          <div className="flex items-center space-x-2 text-sm text-gray-700">
            ...
            <span>{nearbyCaptains.length} drivers available</span>
          </div>
        </div>
